            ...
            <div class="cabecera">
                ...
                <div class="cab-logo">
                    ...
                    <div class="datosCliente">
                        <div class="linea">Cód. Cliente: {{$row->codcli}}</div>
                        <div class="linea"><strong>{{$row->nomcli}}</strong><br>NIF: {{$row->nif}}</div>
                        <div class="linea">{{$row->domici}}<br>{{$row->codpos}} {{$row->poblac}} @if($row->provin), {{SiteHelpers::gridDisplayView($row->provin,'provin','1:paispro:provin:nombre')}} @endif @if($row->pais) ({{SiteHelpers::gridDisplayView($row->pais,'pais','1:paises:pais:nombre')}}) @endif</div>
                    </div>
            ...
            <div class="content">
                <div class="cabFactura">
                    ...
                    <div class="info info-fecha">
                        ...
                        <div class="valueFecha">{{\Carbon\Carbon::parse($row->fecfac)->format("d/m/Y")}}</div>
                    </div>
                    <div style="clear:both;"></div>
                </div>
                ...
                <div class="contentLineasFactura">
                    <table width="100%">
                        ...
                        @if($row->reducida)
                            ...
                            @endforeach
                        @endif
                        @foreach ($subgrid['rowData'] as $index=>$rowLinea)
                            @if(!$row->reducida || ($row->reducida && !array_key_exists(
                                $rowLinea->aplica,
                                ...
                                <tr>
                                    <td width="20%">{{\Carbon\Carbon::parse($rowLinea->feccal)->format("d/m/Y")}}</td>
                                    <td width="40%">{{$rowLinea->descri}}</td>
                                    <td width="10%">{{number_format($rowLinea->unitas, 2, ',', '.')}}</td>
                                    <td width="10%">{{$rowLinea->bastar}}</td>
                                    <td width="10%">{{number_format($rowLinea->import, 2, ',', '.')}}</td>
                                    <td width="10%">{{number_format($rowLinea->seguro, 2, ',', '.')}}</td>
                                </tr>
                                <?php $i++; unset($subgrid['rowData'][$index]);?>
                                    @if($i==\App\Models\factura::LINES_PER_PAGE)
                                        @if(count($subgrid['rowData'])>0)
                ...
                <div class="totales">
                    ...
                    <div style="clear:both;"></div>
                    <div class="cajaTotal cajaValor totalBruto">@if(!$anotherPage){{number_format($row->impbru, 2, ',', '.')}}€@endif</div>
                    <div class="cajaTotal cajaValor descuento">@if(!$anotherPage){{$row->dtofac}}%@endif</div>
                    <div class="cajaTotal cajaValor importeDescuento">@if(!$anotherPage){{number_format($row->impdfac, 2, ',', '.')}}€@endif</div>
                    <div class="cajaTotal cajaValor baseImponible">@if(!$anotherPage){{number_format($row->basiva, 2, ',', '.')}}€@endif</div>
                    <div class="cajaTotal cajaValor porcIVA">@if(!$anotherPage){{$row->tipiva}}%@endif</div>
                    <div class="cajaTotal cajaValor cuotaIVA">@if(!$anotherPage){{number_format($row->impiva, 2, ',', '.')}}€@endif</div>
                    <div class="cajaTotal cajaValor totaFactura">@if(!$anotherPage){{number_format($row->totfac, 2, ',', '.')}}€@endif</div>
                    <div style="clear:both;"></div>

                </div>
                <div class="cajaFormaPago">Forma de pago</div> <div class="formaTransferencia">{{SiteHelpers::gridDisplayView($row->forpag,'forpag','1:forpagos:forpag:descri')}}</div>
            </div>
